...
function getRefs(cat, categorie, dateDebut, dateFin, periodeVote)
{
	var data="cat="+cat+"&categorie="+categorie+"&periodeVote="+periodeVote;
    if(!(dateDebut===null))
    {
    ...
    xmlhttp.open("POST", "indexAjax?module=listeRefs&action=getPartieRefsDeconnecte", true);
...
function getDonnees()
{
    ...
    [cat, categories, dateDebut, dateFin, periodeVote]=etatFormulaire();
    ...
    if(cat!="tous")
    {
        for(var i=0; i<categories.length; i++)
        {
            ...
            getRefs(cat, categorie, dateDebut, dateFin, periodeVote);
        }
    }
    else
    {
        getRefs(cat, "inutile", dateDebut, dateFin, periodeVote); //l'argument categries n'a pas d'intérêt
    }
...
function etatFormulaire()
{
    ...
    return [cat, categories, dateDebut, dateFin, periodeVote];
}
